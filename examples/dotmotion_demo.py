...
class PolkadotOverview(Scene):
    ...
    def ecosystem_demo(self):
        ...
        # Ensure relay label does not overlap titles at the top
        dot.push_below_ring(relay.name, relay.ring, buffer=0.42)
        # Resolve against the title group which sits at the top
        dot.resolve_overlap(relay.name, title_group, direction=DOWN, step=0.12, max_steps=20)
        dot.clamp_to_frame(relay.name, margin=0.25)
        self.wait(0.5)
